...
public class DataLoaderNoLambdas implements DataLoader {
    ...
    public List<Car> loadData() throws IOException, URISyntaxException {
        ...
        for (String line : data) {
            result.add(convertDataLineToCar(line));
        }
        return result;
    }

    private List<String> loadRemoteData() throws IOException {
        return new FileDownloader().getFile("/data.csv");
    }

    private List<String> loadLocalData() throws URISyntaxException, IOException {
        return Files.readAllLines(Paths.get(DataLoaderNoLambdas.class.getResource("/data.csv").toURI()));
    }
